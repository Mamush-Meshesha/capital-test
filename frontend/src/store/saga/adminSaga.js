...
function* adminSignUp(action) {
    try {
        const res = yield call(axios.post, "http://localhost:3000/api/admin/signup", action.payload, {
            headers: {
...
function* adminLogin(action) {
    try {
        const res = yield call(axios.post, "http://localhost:3000/api/admin/login", action.payload, {
            headers: {
                "Content-Type":"application/json"
            },
...
function* adminLogout(action) {
    try {
        const res = yield call(axios.post, "http://localhost:3000/api/admin/logout", action.payload, {
            headers: {
                "Content-Type":"application/json"
            },
            withCredentials: true,
...
export { watchAdminLogin, watchAdminSignup, watchAdminLogout, watchFetchRole, watchFetchCustomer, watchFetchManager, watchFetchRestaurant };